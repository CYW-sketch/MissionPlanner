...
1. **双端同时监听**:
   ...
   - UDP: 主动连接一个端口，同时被动监听另一个端口（默认端口对为 14551/14552）
...
- **质量窗口**: 默认 3 秒（每秒滚动）
- **最小切换间隔**: 默认 10 秒
 - **UDP 双监听端口对**: 14551 / 14552（活跃在哪一端，被动监听自动选择另一端）
...
3. **被动监听**:
   ...
   - UDP: 同时以被动方式在另一 UDP 端口监听（14551/14552 互为对端）
...
2. **端口配置**: 主/备端口可相同或不同；UDP 模式下默认使用 14551/14552 作为端口对，无需额外配置
